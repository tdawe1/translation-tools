...
class PDFBackProjector:
    ...
    def __init__(self, input_path: str, output_path: str, translations_path: str,
        ...
        # Load font configuration from environment or defaults
        self.font_paths = {}

        # Check for environment variable configuration first (JSON string)
        env_fonts = os.getenv("LM_FONT_PATHS")
        if env_fonts:
            try:
                self.font_paths = json.loads(env_fonts)
            except json.JSONDecodeError:
                logging.warning("Invalid JSON in LM_FONT_PATHS environment variable")

        # If not fully configured, fall back to platform defaults
        if not self.font_paths:
            ...
            # Merge defaults, preferring existing env vars if any
            for k, v in defaults.items():
                if k not in self.font_paths:
                    self.font_paths[k] = v

        # Only keep font paths that actually exist
        self.font_paths = {k: v for k, v in self.font_paths.items() if Path(v).exists()}
