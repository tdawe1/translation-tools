...
class PDFTranslationOrchestrator:
    ...
    def _translate_with_cache(self, text_list: List[str]) -> Dict[str, str]:
        """Translate text list using cache and API calls."""
        if not text_list:
            return {}
...
client = GPT5Adapter(api_key=api_key)
                ...
                # Prepare items for batch translation (format expected by PPTX system)
                # batch_translate expects a list of strings, not dictionaries
                items = uncached
                
                # Call batch translation
                translated_list = batch_translate(client, self.model, items, self.glossary, offline_mode=self.offline)
                
                # Update cache with new translations
                for i, text in enumerate(uncached):
                    if i < len(translated_list):
                        translations[text] = translated_list[i]
                        self.cache[text] = translated_list[i]
                
                self.stats['api_calls'] += 1
                self._save_cache()
                logger.info(f"Added {len(translated_list)} new translations to cache")
  ...
  OPENAI_MODEL overrides default model selection
    ...
    if not args.offline and not args.cache_only:
        if not os.getenv("OPENAI_API_KEY"):
            ...
            print("Set it or use --offline or --cache-only mode")
            sys.exit(1)
