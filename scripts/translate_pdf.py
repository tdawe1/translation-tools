...
class PDFTranslationOrchestrator:
    ...
    def __init__(self,
        ...
        Args:
            ...
            do_audit: Whether to generate an audit report
            do_csv: Whether to generate a bilingual CSV
        """
    ...
    def _normalise_key(self, text: str) -> str:
        """Normalize text for consistent cache keys (NFKC, trim whitespace)."""
        import unicodedata
        # NFKC normalization
    ...
    def _load_cache(self) -> Dict[str, str]:
        ...
        if os.path.exists(self.cache_file):
            try:
                ...
                # Normalize keys when loading
                for k, v in raw_cache.items():
                    cache[self._normalise_key(k)] = v
                    # Also keep original key if different, to support legacy/exact matches
                    if k not in cache:
                        cache[k] = v
    ...
    def _save_cache(self) -> None:
        ...
        try:
            # We want to save both normalized keys and original keys if they differ
            # The current in-memory cache might have both
    ...
    def _translate_with_cache(self, text_list: List[str]) -> Dict[str, str]:
        ...
        for text in text_list:
            norm_key = self._normalise_key(text)
            if norm_key in self.cache:
                translations[text] = self.cache[norm_key]
                self.stats['cache_hits'] += 1
                logger.debug(f"Cache hit: '{text[:50]}...'")
            elif text in self.cache: # Legacy lookup
                translations[text] = self.cache[text]
                self.stats['cache_hits'] += 1
                logger.debug(f"Cache hit (legacy): '{text[:50]}...'")
            else:
                uncached.append(text)

        if uncached and not self.offline and not self.cache_only:
            if not OPENAI_AVAILABLE or not PPTX_SYSTEM_AVAILABLE:
                logger.error("Translation requires OpenAI client and PPTX system; returning cache hits only")
                # Keep existing cache hits, leave uncached as JP for this run
            ...
            try:
                    ...
                    for future in as_completed(future_to_batch):
                        ...
                        try:
                            ...
                            for text, translation in zip(batch_items, batch_results):
                                ...
                                self.cache[self._normalise_key(text)] = translation
                                # Also cache original key to be safe
                            ...
                            # Fallback to originals for this run, but do not cache failures
                ...
                # Fallback: keep cache hits; use originals for uncached this run
                for text in uncached:
                    translations.setdefault(text, text)
...
Environment:
  ...
  OPENAI_MODEL overrides default model selection
    ...
    if not args.offline and not args.cache_only:
        api_key = os.getenv("OPENAI_API_KEY") or os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
        if not api_key:
        ...
        # Validate key matches model
        if "gemini" in args.model.lower():
            gemini_key = os.getenv("GOOGLE_API_KEY") or os.getenv("GEMINI_API_KEY")
            if not gemini_key:
                print("Error: Gemini model selected but no GOOGLE_API_KEY or GEMINI_API_KEY found.")
                sys.exit(1)
        else:
            openai_key = os.getenv("OPENAI_API_KEY")
            if not openai_key:
                print("Error: OpenAI model selected but no OPENAI_API_KEY found.")
                sys.exit(1)
